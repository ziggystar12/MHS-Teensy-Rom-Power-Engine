...
#include <vector>
#include <fstream>
#include "../Source/VM/doom/gba/doomvm.cpp"
extern "C" {
#include "../Source/VM/doom/gba/sound.c"
}
...
static bool expectedNtsc, expectedCart;
static uint32_t opens;
static uint16_t acceptedVideoFlags=65535;
static std::vector<uint16_t> videoSetups;
static uint32_t openMusic(const char *name,VmFileInfo *info){assert(!strcmp(name,expectedCart?(expectedNtsc?"/@cart/doom-e1m1-ntsc.s3m":"/@cart/doom-e1m1-pal.s3m"):expectedNtsc?"/VMS/DOOMVM/doom-e1m1-ntsc.s3m":"/VMS/DOOMVM/doom-e1m1-pal.s3m"));opens++;if(missing)return 0;*info={};info->bytes=disk.size();return 7;}
...
int main(int argc,char **argv){
 ...
 h.video_configure=[](const VmIndexedVideoSetup *s){videoSetups.push_back(s->reserved);assert(s->default_mode==0&&s->capabilities==15);return !(s->reserved&~acceptedVideoFlags);};h.video_indexed=[](VmIndexedFrame *f){f->resolved_mode=2;return busy?VmVideoResult::Busy:VmVideoResult::Transferred;};
 for(unsigned old=0;old<4;old++){
  const uint16_t removed[]={0,VM_INDEXED_SOLID_STATUS,VM_INDEXED_SOLID_STATUS|VM_INDEXED_COLOR_F1,VM_INDEXED_SOLID_STATUS|VM_INDEXED_COLOR_F1|VM_INDEXED_STABLE_RASTER};
  acceptedVideoFlags=uint16_t(~removed[old]);
  videoSetups.clear();assert(vm_entry(&h));assert(videoSetups.size()==old+1);
  assert(videoSetups[0]==(VM_INDEXED_SEPARATE_SELECTORS|VM_INDEXED_STABLE_RASTER|VM_INDEXED_COLOR_F1|VM_INDEXED_SOLID_STATUS));
  if(old)assert(videoSetups[1]==(VM_INDEXED_SEPARATE_SELECTORS|VM_INDEXED_STABLE_RASTER|VM_INDEXED_COLOR_F1));
  if(old>=2)assert(videoSetups[2]==(VM_INDEXED_SEPARATE_SELECTORS|VM_INDEXED_STABLE_RASTER));
  if(old==3)assert(videoSetups[3]==VM_INDEXED_SEPARATE_SELECTORS);
 }
 acceptedVideoFlags=65535;puts("PASS Doom Color F1/status negotiation and three older-firmware fallbacks");
 ...
 // Verify current CRC lookup against the previous bitwise implementation.
 for(unsigned n=0;n<4096;n++){std::vector<uint8_t> bytes(n);for(unsigned i=0;i<n;i++)bytes[i]=uint8_t(i*37+n);
  uint16_t expected=65535;for(auto byte:bytes){expected^=uint16_t(byte)<<8;for(unsigned bit=0;bit<8;bit++)expected=uint16_t((expected<<1)^((expected&0x8000)?0x1021:0));}
  assert(SidStream::crc(bytes.data(),bytes.size())==expected);
 }
 // Optional music follows the virtual cartridge root for both standards.
 h.package_root="/@cart";expectedCart=true;
 stream.open(&h);assert(stream.handle&&stream.tick(0));stream.close();
 disk[5]=1;headerCrc();expectedNtsc=true;stream.open(&h,true);assert(stream.handle&&stream.tick(0));stream.close();
 h.package_root="/VMS/DOOMVM";expectedCart=false;expectedNtsc=false;disk=good;
 puts("PASS 4096 old/new CRC comparisons and PAL/NTSC virtual-cartridge music roots");
