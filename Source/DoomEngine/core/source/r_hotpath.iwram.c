...
static unsigned short spanstart[MAX_SCREENHEIGHT];                // killough 2/8/98
...
static void R_DrawColumn (const draw_column_vars_t *dcvars)
{
    ...
    byte* dest = drawvars.byte_topleft + ScreenYToOffset(dcvars->yl) + dcvars->x;
...
static void R_DrawFuzzColumn (const draw_column_vars_t *dcvars)
{
    ...
    byte* dest = drawvars.byte_topleft + ScreenYToOffset(dc_yl) + dcvars->x;
...
static void R_DrawMaskedColumn(R_DrawColumn_f colfunc, draw_column_vars_t *dcvars, const column_t *column)
{
    ...
    while (column->topdelta != 0xff)
    {
        ...
        if (yh < viewheight && yl <= yh)
        {
            ...
            fixed_t first = dcvars->texturemid + (dcvars->yl-centery)*dcvars->iscale;
            fixed_t last = dcvars->texturemid + (dcvars->yh-centery)*dcvars->iscale;
            while (dcvars->yl <= dcvars->yh && first < 0) {dcvars->yl++;first += dcvars->iscale;}
            while (dcvars->yl <= dcvars->yh && last >= (column->length<<FRACBITS)) {dcvars->yh--;last -= dcvars->iscale;}
            if (dcvars->yl <= dcvars->yh) colfunc (dcvars);
        }
...
static void R_DrawVisSprite(const vissprite_t *vis)
{
    ...
    const patch_t *patch = GbaFrameLump(vis->lump);
...
static const column_t* R_GetColumn(const texture_t* texture, int texcolumn)
{
    ...
    {
        //simple texture.
        const patch_t* patch = GbaFrameLump(texture->patches[0].lump);
    ...
    else
    {
        ...
        do
        {
            ...
            const patch_t* realpatch = GbaFrameLump(patch->lump);
...
static void R_DrawSprite (const vissprite_t* spr)
{
    ...
    for (const drawseg_t* ds = ds_p; ds-- > drawsegs; )  // new -- killough
    {
        ...
        fixed_t gzt = spr->texturemid + viewz;
...
static void R_DrawPSprite (pspdef_t *psp, int lightlevel)
{
    ...
    const int lump = sprframe->lump[0]+_g->firstspritelump;
    GbaPatchInfo header;GbaPatchHeader(lump,&header);
    const GbaPatchInfo *patch = &header;
...
static void R_DrawSpan(unsigned int y, unsigned int x1, unsigned int x2, const draw_span_vars_t *dsvars)
{
    ...
    byte* dest = drawvars.byte_topleft + ScreenYToOffset(y) + x1;
...
static void R_DoDrawPlane(visplane_t *pl)
{
    ...
    if (pl->minx <= pl->maxx)
    {
        ...
        {     // regular flat
            ...
            dsvars.source = GbaFrameLump(_g->firstflat + flattranslation[pl->picnum]);
...
static void R_ProjectSprite (mobj_t* thing, int lightlevel)
{
    ...
    const int lump = sprframe->lump[rot] + _g->firstspritelump;
    GbaPatchInfo header;GbaPatchHeader(lump,&header);
    const GbaPatchInfo *patch = &header;
    ...
    vis->iscale = FixedDiv(tz, projectiony);
    vis->lump = lump;
    vis->gx = fx;
    vis->gy = fy;
    vis->gz = fz;
...
int I_GetTime(void) { return GbaClockTics(); }
