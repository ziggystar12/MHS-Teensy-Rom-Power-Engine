...
static void GbaLoadLevelOnce(void)
{
    // Set the sky map.
...
void G_DoCompleted (void)
{
    ...
    if (_g->automapmode & am_active) AM_Stop();
    if (_g->gamemap == 9) _g->player.didsecret = true;
    _g->totalleveltimes += _g->leveltime - _g->leveltime % 35;
    if (_g->gamemap == 8) { GbaFinishEpisode(); return; }
    _g->gamemap = _g->secretexit ? 9 : _g->gamemap == 9 ? 4 : _g->gamemap + 1;
    G_DoLoadLevel();
}
