...
export function emitVideoClient(e,state,stage,captureInput='nes_capture_input',cropState=null){
  ...
  e.label('mpe_pause_hidden');e.emit(0x78);set(0xd01a,0);set(0xd015,0);set(sprites,0);set(0xd011,0x2b);
  if(cropState)set(cropState,0);
  ...
  // New negotiated F5: flag 4 means ordinary hires plus sprite plane. Keep
  // enabled exclusively for the timed kernel; old flags 0/1 retain meaning.
  if(cropState)set(cropState,0);
  set(sprites,0);get(stage+10);e.emit(0xc9,4);e.branch(0xd0,'mpe_resume_crop_flags');
  get(stage+9);e.emit(0xc9,2);e.jumpUnless(0xf0,'error_type');set(sprites,1);e.emit(0xa9,0);e.abs(0x4c,'mpe_resume_flags_ready');
  // Negotiated F3 crop uses the ordinary multicolor IRQ. Only this ACKed
  // profile enables camera bits; an older host keeps receiving legacy input.
  e.label('mpe_resume_crop_flags');e.emit(0xc9,8);e.branch(0xd0,'mpe_resume_legacy_flags');
  get(stage+9);e.emit(0xc9,1);e.jumpUnless(0xf0,'error_type');
  if(cropState)set(cropState,1);
  e.emit(0xa9,0);e.abs(0x4c,'mpe_resume_flags_ready');
  e.label('mpe_resume_legacy_flags');e.emit(0xc9,2);e.jumpUnless(0x90,'error_type');
  e.label('mpe_resume_flags_ready');put(enabled);
  ...
  set('color_destination_page',0xd8);get(stage+10);e.emit(0xc9,8);e.branch(0xf0,'mpe_resume_color');
  get(stage+9);e.branch(0xf0,'mpe_resume_color');e.emit(0xa9,8);e.branch(0xd0,'mpe_resume_mode');
  ...
  e.label('mpe_video_disable');if(cropState)set(cropState,0);
  get(enabled);e.abs(0x0d,activeBank,'read');e.abs(0x0d,streaming,'read');e.abs(0x0d,sprites,'read');e.branch(0xf0,'mpe_video_disabled');e.abs(0x20,'wait_fresh_border');e.emit(0x08,0x78);
  set(streaming,0);set(flipPending,0);set(activeBank,0);get(0xdd00);e.emit(0x29,0xfc,0x09,2);put(0xdd00);
  set(enabled,0);set(sprites,0);set(0xd015,0);vector('raster_irq');set(0xd012,250);set(0xd011,0x3b);set(0xd018,0x78);set(0xd019,1);e.emit(0x28);
  e.label('mpe_video_disabled');e.emit(0x60);

  e.label('mpe_video_sprites');set(0xd017,0);set(0xd01b,0);set(0xd01c,0);set(0xd01d,0);
  e.emit(0xa2,15);e.label('mpe_sprite_positions');e.abs(0xbd,0x5a00,'read');e.abs(0x9d,0xd000,'write');e.emit(0xca);e.branch(0x10,'mpe_sprite_positions');
  e.emit(0xa2,7);e.label('mpe_sprite_colors');e.abs(0xbd,0x5a12,'read');e.abs(0x9d,0xd027,'write');e.emit(0x8a,0x18,0x69,0x60);e.abs(0x9d,0x5ff8,'write');e.emit(0xca);e.branch(0x10,'mpe_sprite_colors');
  get(0x5a10);put(0xd010);get(0x5a11);put(0xd015);e.emit(0x60);
