...
// MPE port: byte accesses honor RAM mirroring and 16-bit address wrap.
// Avoid unaligned reads, cross-bank overreads, and the $FF zero-page wrap bug.
...
{
   return ram[address] | (uint32(ram[uint8(address + 1)]) << 8);
}
INLINE uint32 bank_readword(uint32 address)
{
   return bank_readbyte(address) | (uint32(bank_readbyte(address + 1)) << 8);
}
...
{
   return mem_readbyte(address & 0xFFFF);
}
...
{
   uint32 old_cycles = cpu.total_cycles;
