...
#include "../../Teensy/MinimalBoot/Common/VMRegistry.h"
...
int main(int argc,char **argv){
    ...
    fs::copy_file(fs::path(argv[1])/"DOOMVM.crt",base/"DOOMVM.crt",fs::copy_options::overwrite_existing);
    using namespace VmRegistry;Launch l{};assert(find("gbd",nullptr,l)==1);assert(!strcmp(l.root,"/VMS/DOOMVM"));assert(preflight(l));
    assert(validExtensions("one,two")&&extensionMatches("one,two","ONE")&&extensionMatches("one,two","TWO"));
    assert(!extensionMatches("one,two","g")&&!validExtensions("one,crt")&&!validExtensions("one,")&&!validExtensions(",one"));
    Manifest m{};assert(readManifest(l.root,m));refresh(true);assert(associated("Test.GBD"));assert(!associated("Test.gbd.exe"));refresh(false);assert(!associated("Test.GBD"));
    assert(!absolute("/VMS/../secret",80));assert(!component("../TEST"));assert(!component("TEST/VM"));
    assert(tryLaunch(rmtSD,"/","DOOMVM.crt"));assert(rebooted&&marker=="@VM1");Launch saved{};assert(consume(saved)&&!saved.content[0]);
    rebooted=false;assert(tryLaunch(rmtSD,"/VMS/DOOMVM","doom1.gbd"));assert(rebooted);assert(consume(saved));assert(!strcmp(saved.content,"/VMS/DOOMVM/doom1.gbd"));
    put(base/"VMS/OTHER/manifest.vmi","VM1\nOTHER\ngbd\nengine.mvm\nclient.crt\nEND\n");assert(find("gbd",nullptr,l)==-1);
    put(base/"VMS/OTHER/manifest.vmi","VM1\nOTHER\nother\nengine.mvm\nclient.crt\nEND\n");assert(find("gbd",nullptr,l)==1);
    put(base/"VMS/OTHER/manifest.vmi","VM1\nOTHER\nother\n../bad\nclient.crt\nEND\n");assert(!readManifest("/VMS/OTHER",m));
    auto module=base/"VMS/DOOMVM/engine.mvm";std::fstream damage(module,std::ios::binary|std::ios::in|std::ios::out);damage.seekg(64);const int byte=damage.get();damage.seekp(64);damage.put(byte^1);damage.close();assert(!preflight(l));
    rebooted=false;assert(tryLaunch(rmtSD,"/","DOOMVM.crt"));assert(!rebooted&&!message.empty());
    puts("PASS: real registry/preflight, generic content routing, client launch, one-shot record, duplicate extension, traversal, malformed manifest and corrupt module rejection");
}
